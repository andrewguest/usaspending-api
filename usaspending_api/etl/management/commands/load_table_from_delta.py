...
class Command(BaseCommand):
    ...
    Default (full reload) mode: copies the Delta table into a temp Postgres table. If the table with the chosen
    temp name already exists, all existing data will be deleted before new data is written. Note this only loads
    in the data without accounting for the metadata, so make sure to run the command `copy_table_metadata` after
    this is complete if a new table has been made.

    Incremental mode (--incremental): applies the Delta table's Change Data Feed (CDF) directly to the live
    Postgres table. Reads the last-processed version from cdf_version_tracking, loads only the CDF entries after
    that version, stages them to tables in the temp schema, and applies the changes together with the
    tracking-table update in a single transaction. Does not seed the initial tracking record — a full reload
    must run first and the tracking record must be seeded by the operator.
    """
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        ...
        parser.add_argument(
            "--incremental",
            action="store_true",
            help="Run in incremental mode: apply the Delta table's Change Data Feed (CDF) to the live Postgres "
            "table instead of doing a full reload to a temp table. Requires that the TABLE_SPEC entry defines a "
            "primary_key_column and that an initial tracking record has been seeded after a prior full reload.",
        )
        parser.add_argument(
            "--cleanup-staging",
            action="store_true",
            help="Only valid with --incremental. Drop the CDF staging tables after a successful apply. "
            "Default keeps them for audit.",
        )
    ...
    def _recreate_cdf_staging(self, cursor, deletes_staging, upserts_staging, live_table, pk_column):
    ...
    def _apply_cdf_upserts(self, cursor, live_table, upserts_staging, column_names) -> int:
